# Skills 명세 인덱스 — financial-dashboard v3.0.0 (FINAL)

> Omni-Dash 금융 투자 대시보드 자동 생성 스킬 시스템의 **최종 제출 명세**.
> 6개 역할별 모듈 + INDEX + CHANGELOG + 5개 골든 examples로 구성되며, 해커톤 제출 규정 "1개 또는 여러 개 제출 가능 / 기능별·역할별 구분 / .zip 제출"을 따른다.
>
> **제출 형식**: 본 디렉토리(`docs/final_skills/`) 전체를 `skills.zip`으로 압축.
> ```bash
...
| # | 파일 | 역할 (한 줄) | 라인 |
|---|---|---|---|
| 0 | [`00_core_skills.md`](./00_core_skills.md) | 진입점·파이프라인·출력 모드·audience 결정·전역 가드레일·트리거 키워드 | 143 |
| 1 | [`01_data_skills.md`](./01_data_skills.md) | 데이터 정규화·거래내역 변환·입력 어댑터·다중 포트폴리오 병합 | 163 |
| 2 | [`02_metrics_skills.md`](./02_metrics_skills.md) | 시맨틱 레이어 (수익률·위험 지표)·자산군별 차등 임계·audience 노출 정책 | 143 |
| 3 | [`03_visualization_skills.md`](./03_visualization_skills.md) | 차트 선택 매트릭스·색상 거버넌스·Z-Layout·라이브러리 매핑 | 137 |
| 4 | [`04_report_skills.md`](./04_report_skills.md) | BUILD 5단 프레임워크·NLG 인사이트·임계 트리거·초보자 용어 사전 | 152 |
| 5 | [`05_contract_skills.md`](./05_contract_skills.md) | TypeScript 출력 계약·모드별 채움·캐시·핫리로드·E2E 예시 | 367 |
| — | [`INDEX.md`](./INDEX.md) | 본 인덱스 (모듈 지도·의존 그래프·빠른 참조·제출 가이드) | 본 파일 |
| — | [`CHANGELOG.md`](./CHANGELOG.md) | 버전 변경 이력 (SemVer, v1.0.0 → v3.0.0) | 199 |
| — | [`examples/`](./examples/) | 골든 케이스 5종 입력 + 2종 기대 출력 JSON (자동 검증 케이스) | — |

**합계**: ~1,400라인 (8 파일) + 7 examples 파일

## examples/ 골든 케이스

| 파일 | 시연 시나리오 | 검증 영역 |
|---|---|---|
| [`01_kr_equity_simple.csv`](./examples/01_kr_equity_simple.csv) | 시연 1: 국내 주식 단일 자산군 | 한국어 컬럼 헤더 자동 매핑, 단일 자산군 임계 |
| [`02_global_etf.csv`](./examples/02_global_etf.csv) | 시연 1·2: 해외 ETF 분산 | 다중 통화·영문 헤더 |
| [`03_multi_asset.csv`](./examples/03_multi_asset.csv) | 시연 2: 주식+ETF+채권+암호 혼재 | 자산군별 차등 임계의 효과 검증 |
| [`04_transactions.csv`](./examples/04_transactions.csv) | 거래내역 입력 | §1.2.2 거래내역 → 잔고 변환 + 세후/세전 분리 |
| [`05_messy_input.csv`](./examples/05_messy_input.csv) | 시연 1: 양식 엉망 입력 | §1.4 입력 어댑터 자동 매핑 + 결측치 처리 |
